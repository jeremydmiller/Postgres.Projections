...
exports.seedAll = function(options){
	return execute(options.connection, [
		loadFile('schema.sql'), 
...
var buildStorageDDL = function(options){
	var projector = require('./pg-events');
	var DDLGenerator = require('./ddl-generator');

	projector.loadProjectionsFromFolder(options.projection_folder);

 	return new DDLGenerator(projector);
};

// look for file: as an option
exports.generateDDL = function(options){
 	return buildStorageDDL(options).generate();
};



// TODO -- rebuild projections from scratch
